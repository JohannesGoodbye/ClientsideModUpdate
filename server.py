...
NETLIFY_SERVER_MODS_URL = NETLIFY_BASE_URL + "/modfiles/server"
SERVER_MODLIST_URL = NETLIFY_SERVER_MODS_URL + "/modlist.txt"
...
def download_mod(mod_filename, environment):
    ...
    if environment == "common":
        ...
        mod_url = f"{NETLIFY_SERVER_MODS_URL}/{mod_filename}"
...
def update_mods():
    ...
    cloud_server_mods = get_cloud_modlist("server")
    cloud_mods.update(cloud_server_mods)
    ...
    if not installed_mods:
        ...
        print("Downloading mods.zip (server)")
        download_and_extract_zip("mods.zip", "server")
